...
function processLineAsset(line) {
    return { ...line, lines: [], isFolded: true };
}

function extractPrintDataAsset(lines) {
    const data = [];
    for (const line of lines) {
        ...
        if (!line.isFolded) {
            data.push(...extractPrintDataAsset(line.lines));
        }
    }
    return data;
}

export class AssetTraceabilityReport extends Component {

    static template = "sttl_assets.TraceabilityReportAsset";
    ...
    async onWillStart() {
        ...
        if (!this.state.lines.length) {
            const mainLines = await this.orm.call("stock.traceability.report", "get_main_lines_assets", [
                this.context,
            ]);
            this.state.lines = mainLines.map(processLineAsset);
            console.log(this.state.lines, "lines");
    ...
    onCLickOpenLot(line) {
        this.actionService.doAction({
            ...
            tag: "stock_traceability_report_asset",
    ...
    onClickPrint() {
        const data = JSON.stringify(extractPrintDataAsset(this.state.lines));
    ...
    async toggleLine(line) {
        ...
        if (!line.lines.length) {
            ...
            ).map(processLineAsset);
        }
    }
}

registry.category("actions").add("stock_traceability_report_asset", AssetTraceabilityReport);
